...
const cipher = {
    encode: (offset, string) => {
        if (offset == 'string' || offset == null || string == []) {
            throw TypeError("The input should be numeric.")
        }
        ...
        for (let i = 0; i < string.length; i++) {
            ...
            if (unicodeCharacter >= 32 && unicodeCharacter <= 64) { // signos, numeros, operadores y caracteres especiales
                let letter = String.fromCharCode((unicodeCharacter - 32 + parseInt(offset)) % 33 + 32);
                ...
                let letter = String.fromCharCode((unicodeCharacter - 123 + parseInt(offset)) % 3 + 123);
                encryptedMessage = encryptedMessage + letter;
            } else {
    ...
    decode: (offset, string) => {
        ...
        for (let i = 0; i < string.length; i++) {
            ...
            if (unicodeCharacter >= 32 && unicodeCharacter <= 64) {
                let letter = String.fromCharCode((unicodeCharacter - 64 - parseInt(offset)) % 33 + 64);
                encryptedMessage = encryptedMessage + letter;
                ...
                let letter = String.fromCharCode((unicodeCharacter - 126 - parseInt(offset)) % 3 + 126);
                encryptedMessage = encryptedMessage + letter;
            } else {
                encryptedMessage = encryptedMessage + string[i];
            }
